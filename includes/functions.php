...
/**
 * Zkontroluje, zda JSON s textem písně obsahuje nějaké štítky (čipy).
 * @param string|null $lyrics_json JSON řetězec z databáze.
 * @return bool True, pokud byl nalezen alespoň jeden štítek.
 */
function has_chips($lyrics_json) {
    if (empty($lyrics_json)) return false;
    $lyrics = json_decode($lyrics_json, true);
    if (!is_array($lyrics)) return false;
