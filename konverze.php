...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fetch_url'])) {
    ...
    if (!filter_var($url, FILTER_VALIDATE_URL)) {
        ...
        if (!$html) {
            ...
            // Performer je na velkyzpevnik.cz obvykle v <h3> (někdy mohl být v <h2>, tak zkusíme oba)
            $performer_node = $xpath->query('//h3/a[@title="Detail interpreta"]')->item(0) 
                             ?? $xpath->query('//h3')->item(0) 
                             ?? $xpath->query('//h2')->item(0);
            
            if ($performer_node) {
                $raw_performer = trim($performer_node->nodeValue);
                // Pokud je tam čárka (např. "Matuška, Waldemar"), zkusíme to prohodit a dát do 'artist'
                if (strpos($raw_performer, ',') !== false) {
                    $parts = explode(',', $raw_performer);
                    $swapped = trim($parts[1] ?? '') . ' ' . trim($parts[0]);
                    $form_data['artist'] = trim($swapped);
                } else {
                    // Pokud tam čárka není, dáme to do 'band' (kapela)
                    $form_data['band'] = $raw_performer;
                }
            }
