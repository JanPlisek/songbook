...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_song'])) {
    ...
    try {
        ...
        $pdo->commit();
        $message = "Úspěch! Písnička '" . htmlspecialchars($title) . "' byla uložena.";
        $message_type = 'success';
        if (empty($_POST['id'])) { // Vyčistíme formulář jen při tvorbě nové písně
            $form_data = ['id' => null, 'title' => '', 'artist' => '', 'band' => '', 'key' => '', 'capo' => 0, 'raw_content' => ''];
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (songTextarea) {
        const editor = CodeMirror.fromTextArea(songTextarea, {
            lineNumbers: true, lineWrapping: true, mode: 'text/plain', theme: 'material-darker',
