...
<div class="song-page">
    <div id="song-controls-container">
        <div id="controls-popup">
            ...
            <button id="btn-toggle-chords" title="Schovat akordy"><span class="material-symbols-outlined">visibility_off</span></button>
            ...
            <hr>
            <button id="btn-show-chords" title="Nápověda akordů"><span class="material-symbols-outlined">school</span></button>
        </div>
...
<div id="chord-modal" class="modal-overlay" style="display: none;">
    <div class="modal-content">
        ...
        <div id="chord-modal-content" class="chord-modal-grid">
            <!-- Dynamicky plněno -->
        </div>
        <button id="btn-close-chord-modal" class="btn-close-modal">Zavřít</button>
    </div>
</div>
<?php include 'includes/footer-song.php'; ?>

<script src="assets/js/chord-visualizer.js"></script>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        const createHtmlForLines = (lines) => {
            ...
            lines.forEach(line => {
                ...
                const formatLabel = (label) => {
                    ...
                    if (l === 'R' || l === 'REF') return 'Ref.';
                    if (l.startsWith('R') && l.length > 1) {
                         const num = l.replace(/[^0-9]/g, '');
                         return num ? `Ref. ${num}` : 'Ref.';
                    }
                    if (l === 'INT' || l === 'INTRO') return 'INTRO';
                    if (l === 'MEZI') return 'MEZIHRA';
                    if (l === 'OUT' || l === 'OUTRO') return 'OUTRO';
                    if (!isNaN(label)) return label + '.';
                    return label.toUpperCase();
                };

                // Přidání štítku sloky jako "chip" nad sloku
                if (line.label) {
                    html += `<div class="stanza-label-wrapper"><span class="stanza-label">${formatLabel(line.label)}</span></div>`;
                }

                // Vykreslíme řádek s textem/akordy pouze pokud není prázdný u značek
                // (Zabráníme prázdnému řádku pod samostatným čipem např. pro opakovaný refrén)
                const hasContent = (line.text && line.text.trim() !== '') || (line.chords && line.chords.trim() !== '');
                if (hasContent || !line.label) {
                    // fallback: odstraníme značky '!' a '.' pokud by náhodou zůstaly v datech
                    ...
                    let cleanChords = line.chords || '';
                    if (typeof cleanChords === 'string' && (cleanChords.startsWith('.') || cleanChords.startsWith('!'))) {
                        cleanChords = cleanChords.substring(1).trim();
                    }

                    html += `<div class="${groupClass}">`;
                    if (cleanChords && cleanChords.trim() !== '') { html += `<div class="chords-line">${cleanChords}</div>`; }
                    html += `<div class="text-line">${cleanText}</div>`;
                    html += '</div>';
                }
            });
            return html;
    ...
    // OVLÁDACÍ PANEL
    let currentFontSize = lyricsContainer ? parseFloat(window.getComputedStyle(lyricsContainer).fontSize) : 14;
    const updateFontSize = () => { 
        if(lyricsContainer) {
            lyricsContainer.style.fontSize = `${currentFontSize}px`; 
            // Vynutíme překreslení pro stabilitu na některých mobilních prohlížečích
            lyricsContainer.style.display = 'none';
            lyricsContainer.offsetHeight; // trigger reflow
            lyricsContainer.style.display = 'grid';
        }
    };
    document.getElementById('btn-increase-font')?.addEventListener('click', (e) => { 
        e.preventDefault();
        currentFontSize += 2; 
        updateFontSize(); 
    });
    document.getElementById('btn-decrease-font')?.addEventListener('click', (e) => { 
        e.preventDefault();
    ...
    document.getElementById('btn-reset-font')?.addEventListener('click', (e) => { 
        e.preventDefault();
        lyricsContainer.style.fontSize = ''; 
        currentFontSize = parseFloat(window.getComputedStyle(lyricsContainer).fontSize);
    });
    ...
    if (toggleChordsBtn) {
        toggleChordsBtn.addEventListener('click', () => {
            const isNoChords = lyricsContainer.classList.toggle('lyrics-no-chords');
            const icon = toggleChordsBtn.querySelector('.material-symbols-outlined');
            if (isNoChords) {
                icon.textContent = 'visibility';
                toggleChordsBtn.title = 'Zobrazit akordy';
            } else {
                icon.textContent = 'visibility_off';
                toggleChordsBtn.title = 'Schovat akordy';
            }
        });
    }
    ...
    document.querySelectorAll('.transpose-keys button').forEach(b => { 
        b.addEventListener('click', () => { 
            ...
            // Pro výpočet posunu použijeme aktuální tóninu
            ...
            // AKTUALIZACE DAT: Přepíšeme songLyricsData novými akordy, 
            // aby příští transpozice vycházela z tohoto nového stavu.
            for (let i = 0; i < songLyricsData.length; i++) {
                songLyricsData[i].chords = transposeLine(songLyricsData[i].chords, transposeAmount);
            }
    ...
    const chordModalContent = document.getElementById('chord-modal-content');

    const showChordModal = async () => {
        if (!window.ChordVisualizer.chordData) {
            await window.ChordVisualizer.init();
        }

        // Získáme unikátní akordy z dat písně
        const uniqueChords = new Set();
        songLyricsData.forEach(line => {
            if (line.chords) {
                // Rozdělíme řádek s akordy na jednotlivé kusy
                const chords = line.chords.split(/[ \(\)\/\-]+/); 
                chords.forEach(c => {
                    const trimmed = c.trim();
                    if (trimmed && !trimmed.includes('!')) {
                        // Ignorujeme značky opakování, čísla a samotné tečky/čárky
                        if (/^[0-9]+x?$/i.test(trimmed) || /^[\.,]+$/.test(trimmed)) return;
        ...
        if (uniqueChords.size === 0) {
            chordModalContent.innerHTML = '<p>V této písni nebyly detekovány žádné akordy.</p>';
        } else {
            let html = '';
            Array.from(uniqueChords).sort().forEach(chordName => {
                const chordInfo = window.ChordVisualizer.getChordInfo(chordName);
                if (chordInfo) {
                    html += `<div class="chord-item">` + window.ChordVisualizer.render(chordName, chordInfo) + `</div>`;
                } else {
                    // Fallback pro neznámé akordy
                    html += `<div class="chord-item unknown"><div class="chord-name-only">${chordName}</div><div class="chord-error">Neznámý hmat</div></div>`;
                }
            });
            chordModalContent.innerHTML = html;
        }

        chordModal.style.display = 'flex';
    };

    document.getElementById('btn-show-chords')?.addEventListener('click', (e) => {
        e.preventDefault();
        showChordModal();
    });

    document.getElementById('btn-close-chord-modal')?.addEventListener('click', () => {
    ...
    // PRVNÍ SPUŠTĚNÍ
    renderSong(songLyricsData);
    // Na mobilu žádné automatické zmenšování - spoléháme na čisté CSS a zalamování
    if (window.innerWidth >= 992) {
        autofitFontSize();
    }
    window.addEventListener('resize', () => {
        if (window.innerWidth >= 992) autofitFontSize();
    });
