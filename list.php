...
$sql = "SELECT 
        ...
        GROUP BY 
            s.id
        ORDER BY
            -- Krok 1: Vytvoříme dvě skupiny - písmena (0) a zbytek (1) a seřadíme je.
            CASE
                WHEN s.title REGEXP '^[[:alpha:]]' THEN 0
                ELSE 1
            END,
            -- Krok 2: V rámci těchto skupin seřadíme normálně abecedně.
        ...
        <div class="right-group">
            ...
            if (is_admin()): ?>
                <?php if ($pending_requests_count > 0): ?>
                    <span> | </span>
                    <a href="admin_settings.php?tab=requests" class="requests-link">
                            ...
                            <div class="song-list-item-wrapper" data-song-id="<?php echo htmlspecialchars($song['id']); ?>">
                                ...
                                <div class="song-list-item">
                                    <a href="song.php?id=<?php echo htmlspecialchars($song['id']); ?>" class="song-title-link">
                                        ...
                                        <?php if (is_admin() && !has_chips($song['lyrics'])): ?>
                                            <span class="no-chips-indicator" title="Píseň nemá sloupce/čipy (není upravena)">⚠️</span>
                                        <?php endif; ?>
                                    </a>
